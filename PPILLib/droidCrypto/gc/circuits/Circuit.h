...
class Circuit {
 public:
  ...
  void garble(const std::vector<BitVector> &database);
  ...
  std::chrono::duration<double> timeOutput;
    std::chrono::duration<double> timeNetworkCircuit;
  ...
  int bytesSentTotal;
    ...
    std::vector<BitVector> evaluate(const BitVector &fingerprint, int m, CommunicationHandler& comm);

    void garble(const std::vector<BitVector> &database, int protNum, CommunicationHandler& comm);
    std::vector<BitVector> evaluate(const std::vector<BitVector> &distanceShares, CommunicationHandler& comm);
...
protected:
  ...
  virtual  std::vector<std::vector<WireLabel>> computeFunction(
          const std::vector<std::vector<WireLabel>> &inputA,
          const std::vector<WireLabel> &inputB, GCEnv &env) = 0;
  virtual  std::vector<std::vector<WireLabel>> computeFunction(const std::vector<std::vector<WireLabel>> &inputServer,
  ...
  const size_t mOutput_size;
    ...
    evaluate(const std::vector<DeltaShare> &distanceShares, droidCrypto::AEnv &aEnv);
};
