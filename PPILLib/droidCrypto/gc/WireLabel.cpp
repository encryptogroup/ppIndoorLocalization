...
namespace droidCrypto {
    ...
    SIMDWireLabel::SIMDWireLabel(const std::vector<WireLabel>& vector) {
        for (int i = 0; i < vector.size() ; ++i) {
            bytes.push_back(vector[i].bytes);
        }
    }
    ...
    std::vector<WireLabel> SIMDWireLabel::unwrap() const {
        std::vector<WireLabel> wireVector;
        for(int i = 0; i < bytes.size(); i++){
            WireLabel wl(bytes[i]);
            wireVector.push_back(wl);
        }
        return wireVector;
    }
}