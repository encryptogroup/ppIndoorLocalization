...
void CSocketChannel::closeChannel() {
    close(serversocket);
    close(csocket);
}
...
void CSocketChannel::send(std::vector<droidCrypto::DeltaShare> share) {
    std::vector<block> sendingBlock;
    for (int j = 0; j < share.size(); j++) {
        uint8_t shareValue = share[j].getCapitalDelta();
        sendingBlock.push_back(droidCrypto::toBlock(&shareValue));
    }
    send(sendingBlock);
}

std::vector<uint8_t> CSocketChannel::recv(int length) {
    std::vector<uint8_t> shares;
    std::vector<block> sendingBlock(length);
    recv(sendingBlock);
    //droidCrypto::Log::v("CW", "Received vector");
    for (int j = 0; j < sendingBlock.size(); j++) {
        uint8_t *value = (uint8_t *) &sendingBlock[j];
        shares.push_back(*value);
        //droidCrypto::Log::v("CW","value at %d: %d", j, *value);
    }
    return shares;
}
